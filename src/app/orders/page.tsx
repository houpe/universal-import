    ...
    <div className="max-w-[1600px] mx-auto animate-fadeIn">
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-lg border border-slate-200/60 p-5 mb-6 shadow-sm">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div>
            ...
            <input
              ...
              placeholder="搜索外部编码"
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#2563EB] focus:border-transparent"
            />
          </div>
          <div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button
            ...
            className="px-5 py-2 text-sm font-medium text-white bg-[#2563EB] hover:bg-[#1D4ED8] rounded-lg hover:shadow-sm transition-all duration-200"
          >
            搜索
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-lg border border-slate-200/60 p-16 text-center shadow-sm">
          ...
          <a href="/import" className="text-sm text-[#2563EB] hover:text-[#1D4ED8] font-medium">
            去导入文件 →
          </a>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {result.data.map((order) => (
                  <tr key={order.id} className="border-b border-gray-50 hover:bg-[#2563EB]/10 transition-colors">
                    ...
                    <td className="px-3 py-2.5">
                      <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium
                        ${order.temp_zone === '常温' ? 'bg-green-100 text-green-700' :
                          order.temp_zone === '冷藏' ? 'bg-[#2563EB]/20 text-[#1D4ED8]' :
